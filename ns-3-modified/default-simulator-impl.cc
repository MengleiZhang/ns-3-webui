...
DefaultSimulatorImpl::DefaultSimulatorImpl()
{
    ...
    m_os.open("simulation_status.txt");
}
...
{
    if(m_simStopTime.GetMilliSeconds() > 0 && Now().GetMilliSeconds() != m_lastTraceTime.GetMilliSeconds())
    {
        m_os << Now().GetMilliSeconds() << "," << m_simStopTime.GetMicroSeconds() << std::endl;
        m_lastTraceTime = Now();
    }
    Scheduler::Event next = m_events->RemoveNext();
